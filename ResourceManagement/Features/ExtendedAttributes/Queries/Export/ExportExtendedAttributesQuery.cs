...
using ResourceManagement.Specifications.ExtendedAttribute;
using Shared.Utilities;
using System.Globalization;

namespace ResourceManagement.Features.ExtendedAttributes.Queries.Export
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        public async Task<Result<string>> Handle(ExportExtendedAttributesQuery<TId, TEntityId, TEntity, TExtendedAttribute> request, CancellationToken cancellationToken)
        {
            ...
            if (request.IncludeEntity)
            {
                ...
                mappers.Add(_localizer["EntityCreatedOn (Local)"], item => item.Entity.CreatedAt.ToString("G", CultureInfo.CurrentCulture));
                mappers.Add(_localizer["EntityCreatedOn (UTC)"], item => DateTime.SpecifyKind(item.Entity.CreatedAt, DateTimeKind.Utc).ToLocalTime().ToString("G", CultureInfo.CurrentCulture));
