...
using ResourceManagement.Enums;
using ResourceManagement.Repositories;
using Shared.Utilities;

namespace LoanManagementSystem.Application.Features.ExtendedAttributes.Commands.AddEdit
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        {
            if (await _unitOfWork.Repository<TExtendedAttribute>().Entities.Where(x => x.EntityId != null && !x.Id.Equals(command.Id) && x.EntityId.Equals(command.EntityId))
            ...
            if (command.Id.Equals(default))
            {
                ...
                await _unitOfWork.CommitAndRemoveCache(cancellationToken, ResourceManagement.Constants.ApplicationConstants.Cache.GetAllEntityExtendedAttributesCacheKey(typeof(TEntity).Name));
                ...
                var cacheKeys = await _unitOfWork.Repository<TExtendedAttribute>().Entities.Select(x =>
                    ApplicationConstants.Cache.GetAllEntityExtendedAttributesByEntityIdCacheKey(typeof(TEntity).Name, x.Entity.Id)).Distinct().ToArrayAsync(cancellationToken);
